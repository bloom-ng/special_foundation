<x-admin-layout title="Admin | Projects" page="projects">
            ...
            <section class="container mx-auto py-8">
                <div class="flex gap-y-4 flex-col ">
                    <div class="flex justify-between md:items-center gap-y-4 flex-col md:flex-row w-full">
                        ...
                        <div>
                            <a type="button" href="/admin/project-schedules/create"
                    ...
                    <thead class="">
                        <tr>
                            <th class="border-b border-gray-300 !p-4 pb-8 !text-left ">
                                <p
                                    class="block antialiased font-sans text-left text-xs font-light leading-normal text-blue-gray-900 !font-bold">
                                    ID
                                </p>
                            </th>
                            <th class="border-b border-gray-300 !p-4 pb-8 !text-left">
                        ...
                        <tr>
                            ...
                            <td class="w-1/3 text-left py-3 px-4">
                                <a class="pr-2 text-blue-300 hover:text-blue-500"
                                    href="/admin/project-schedules/{{ $project->id }}/edit">
                                    Edit
                                </a>
                                <form
                                    id="project-delete-{{$project->id}}"
                                    action="/admin/project-schedules/{{$project->id}}"
                                    method="POST"
    ...
    <script>
        @foreach($projects as $project)
            const form{{$project->id}} = document.getElementById('project-delete-{{$project->id}}');
        
            form{{$project->id}}.addEventListener('submit', (e) => {
            e.preventDefault(); 
            const confirmSubmit = confirm('Proceed to delete?');
            if (confirmSubmit) {
                form{{$project->id}}.submit(); 
            }
            });
        @endforeach
      </script>
