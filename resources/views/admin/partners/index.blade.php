<x-admin-layout title="Admin | Partners Logo" page="cms-partners">
            ...
            <section class="container mx-auto py-8">
                <div class="flex gap-y-4 flex-col ">
                    <div class="flex justify-between md:items-center gap-y-4 flex-col md:flex-row w-full">
                        ...
                        <div>
                            <a type="button" href="/admin/cms-data/partners/create"
                                ...
                                <i class="fas fa-plus"></i> Add Partner Logo
                            </a>
                        </div>
            ...
            <div class="bg-white overflow-auto">
                <table class="min-w-full bg-white">
                    ...
                    <tbody class="text-gray-700 text-xs">
                        ...
                        <tr>
                            <td class="w-1/3 text-left py-3 px-4 hidden">
                                {{ $cloud->id }}
                            </td>
                            <td class="w-1/3 text-left py-3 px-4">
                                <img src="{{ Storage::url($cloud->value) }}" class="w-72 h-48 rounded-md">
                                
                            </td>
                            <td class="w-1/3 text-left py-3 px-4">
                                <a class="pr-2 text-blue-300 hover:text-blue-500"
                                    href="/admin/cms-data/partners/{{ $cloud->id }}/edit">
                                    Edit
                                </a>
                            </td>
                        </tr>
                        @foreach ($partners as $partner)
                            <tr>
                                ...
                                <td class="w-1/3 text-left py-3 px-4">
                                    @if ($partner->type == \App\Models\CMS::TYPE_PARTNERS)
                                        <img src="{{ Storage::url($partner->value) }}" class="w-72 h-48 rounded-md">
                                    @elseif ($partner->type == \App\Models\CMS::TYPE_PARTNERS_CLOUD)
                                        <img src="{{ Storage::url($partner->value) }}" class="w-72 h-48 rounded-md">
                                ...
                                <td class="w-1/3 text-left py-3 px-4">
                                    <a class="pr-2 text-blue-300 hover:text-blue-500"
                                        href="/admin/cms-data/partners/{{ $partner->id }}/edit">
                                        Edit
                                    </a>
                                    @if ($partner->type == \App\Models\CMS::TYPE_PARTNERS)
                                    <form id="cms-delete-{{ $partner->id }}"
                                        action="/admin/cms/{{ $partner->id }}" method="POST"
    ...
    <script>
        @foreach ($partners as $partner)
            const form{{ $partner->id }} = document.getElementById('cms-delete-{{ $partner->id }}');

            form{{ $partner->id }}.addEventListener('submit', (e) => {
                e.preventDefault();
                const confirmSubmit = confirm('Proceed to delete?');
                if (confirmSubmit) {
                    form{{ $partner->id }}.submit();
                }
            });
        @endforeach
    </script>
