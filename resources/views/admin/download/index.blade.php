<x-admin-layout title="Admin | Documents" page="downloads">
                    ...
                    <div class="flex justify-between md:items-center gap-y-4 flex-col md:flex-row w-full">
                        <div>
                            ...
                            <p
                                ...
                                See all downloadable files
                            </p>
                        </div>
                        <div>
                            <a type="button" href="/admin/downloads/create"
    ...
    <script>
        @foreach($downloads as $download)
            const form{{$download->id}} = document.getElementById('download-delete-{{$download->id}}');
        
            form{{$download->id}}.addEventListener('submit', (e) => {
            e.preventDefault(); 
            const confirmSubmit = confirm('Proceed to delete?');
            if (confirmSubmit) {
                form{{$download->id}}.submit(); 
            }
            });
        @endforeach
      </script>
