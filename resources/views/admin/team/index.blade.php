<x-admin-layout title="Admin | Team Members" page="cms-teams">
            ...
            <section class="container mx-auto py-8">
                <div class="flex gap-y-4 flex-col ">
                    <div class="flex justify-between md:items-center gap-y-4 flex-col md:flex-row w-full">
                        ...
                        <div>
                            <a type="button" href="/admin/cms-data/teams/create"
                        ...
                        <form method="GET">
                            <div class="lg:w-96">
                                <div class="relative w-full min-w-[200px] h-10">
                                    <div class="grid place-items-center absolute text-blue-gray-500 top-2/4 right-3 -translate-y-2/4 w-5 h-5">
                    ...
                    <thead>
                        <tr>
                            <th class="border-b border-gray-300 !p-4 pb-8 !text-left">
                                <p class="block antialiased font-sans text-xs font-light leading-normal text-blue-gray-900 !font-bold">
                                    Name
                                </p>
                            </th>
                            <th class="border-b border-gray-300 !p-4 pb-8 !text-left">
                    ...
                    <tbody class="text-gray-700 text-xs">
                        @foreach ($teams as $team)
                            <tr>
                                ...
                                <td class="w-1/4 text-left py-3 px-4">
                                    {{ $team->type == \App\Models\CMS::TYPE_TEAM ? 'Team Member' : 'Board Member' }}
                                </td>
                                <td class="w-1/4 text-left py-3 px-4">
                                    <a class="pr-2 text-blue-300 hover:text-blue-500"
                                        href="/admin/cms-data/teams/{{ $team->id }}/edit">
                                        Edit
                                    </a>
                                    <form id="cms-delete-{{ $team->id }}"
                                        action="/admin/cms/{{ $team->id }}" method="POST"
    ...
    <script>
        @foreach ($teams as $team)
            const form{{ $team->id }} = document.getElementById('cms-delete-{{ $team->id }}');

            form{{ $team->id }}.addEventListener('submit', (e) => {
                e.preventDefault();
