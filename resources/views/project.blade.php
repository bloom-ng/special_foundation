<x-guest-layout title="Special Foundation - Project Schedule" page="blog">
    ...
    <div class="flex flex-col py-20 justify-center">
        ...
        <div class="flex lg:flex-col md:flex-col flex-col px-28 pt-20 justify-center">
            <div class="flex lg:flex-row md:flex-row flex-col w-full md:mb-20 lg:mb-20">
                <p class="flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3 md:w-1/3">January  </br>
                    ...
                    <span class="text-sm text-white montserrat-normal bg-[#26225F] lg:mr-14 md:mr-14 py-2 text-center lg:mt-5 md:mt-5 mt-5 px-2">{{ $project->project }}</span>
                    @endforeach
                </p>
            ...
            <div class="flex lg:flex-row md:flex-row  flex-col w-full md:mb-20 lg:mb-20">
                <p class="flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3 md:w-1/3 lg:mt-0 md:mt-0 mt-5">April  </br>
                    @foreach ($currentYearProjects->where('month', 4) as $project)
                    <span class="text-sm text-white montserrat-normal bg-[#26225F] lg:mr-14 md:mr-14 py-2 text-center lg:mt-5 md:mt-5 mt-5 px-2">{{ $project->project }}</span>
                    @endforeach
                </p>
                <p class=" flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3 md:w-1/3 lg:mt-0 md:mt-0 mt-5">May  </br>
                    @foreach ($currentYearProjects->where('month', 5) as $project)
                    <span class="text-sm text-white montserrat-normal bg-[#26225F] lg:mr-14 md:mr-14 py-2 text-center lg:mt-5 md:mt-5 mt-5 px-2">{{ $project->project }}</span>
                    @endforeach </p>
                <p class="flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3 md:w-1/3 lg:mt-0 md:mt-0 mt-5">June  </br>
            ...
            <div class="flex lg:flex-row md:flex-row flex-col w-full md:mb-20 lg:mb-20">
                <p class="flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3 md:w-1/3 lg:mt-0 md:mt-0 mt-5">July  </br>
                    @foreach ($currentYearProjects->where('month', 7) as $project)
                    <span class="text-sm text-white montserrat-normal bg-[#26225F] lg:mr-14 md:mr-14 py-2 text-center lg:mt-5 md:mt-5 mt-5 px-2">{{ $project->project }}</span>
                    @endforeach
                </p>
                <p class="flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3 md:w-1/3 lg:mt-0 md:mt-0 mt-5">August  </br>
                    @foreach ($currentYearProjects->where('month', 8) as $project)
                    <span class="text-sm text-white montserrat-normal bg-[#26225F] lg:mr-14 md:mr-14 py-2 text-center lg:mt-5 md:mt-5 mt-5 px-2">{{ $project->project }}</span>
                    @endforeach
                </p>
                <p class="flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3  md:w-1/3 lg:mt-0 md:mt-0 mt-5">September  </br>
            ...
            <div class="flex lg:flex-row md:flex-row flex-col w-full">
                <p class="flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3 md:w-1/3 lg:mt-0 md:mt-0 mt-5">October  </br>
                    @foreach ($currentYearProjects->where('month', 10) as $project)
                    <span class="text-sm text-white montserrat-normal bg-[#26225F] lg:mr-14 md:mr-14 py-2 text-center lg:mt-5 md:mt-5 mt-5 px-2">{{ $project->project }}</span>
                    @endforeach
                </p>
                <p class="flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3 md:w-1/3 lg:mt-0 md:mt-0 mt-5">November  </br>
                    @foreach ($currentYearProjects->where('month', 11) as $project)
                    <span class="text-sm text-white montserrat-normal bg-[#26225F] lg:mr-14 md:mr-14 py-2 text-center lg:mt-5 md:mt-5 mt-5 px-2">{{ $project->project }}</span>
                    @endforeach
                </p>
                <p class="flex flex-col text-3xl montserrat-bold text-[#25A8D6] lg:w-1/3 md:w-1/3 lg:mt-0 md:mt-0 mt-5">December  </br>
        ...
        <div class="flex justify-between flex-col md:flex-row lg:flex-row mt-20 px-28">
            <a href="/project?year={{$currentYear}}" class=" {{$year == $currentYear ? 'invisible' : ''}}  bg-[#25A8D6] text-white text-base lg:px-5 md:px-5 rounded-full montserrat-bold py-1 lg:mb-0 md:mb-0 mb-5">
              Current Year - {{$currentYear}}
            </a>

            <a href="/project?year={{$nextYear}}" class="{{$year == $nextYear ? 'invisible' : ''}}  bg-[#25A8D6] text-white text-base lg:px-5 md:px-5 rounded-full montserrat-bold py-1">
             Next Year -  {{$nextYear}}
            </a>
        </div>
    </div>
